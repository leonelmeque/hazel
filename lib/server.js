...
const rawUrl = PRIVATE_BASE_URL || (VERCEL_URL ? `https://${VERCEL_URL}` : '')
// Trim stray whitespace + trailing slashes from the env value, or the feed's
// download link ends up malformed (a trailing space → "…app /download/…").
const url = rawUrl.trim().replace(/\/+$/, '') || undefined
