...
module.exports = class Cache {
  ...
  async refreshCache() {
    const { account, repository, pre, token, tagPrefix } = this.config
    ...
    const release = data.find(item => {
      ...
      // Only desktop releases carry macOS assets — skip mobile-vX.Y.Z tags.
      const prefixOk = !tagPrefix || (item.tag_name || '').startsWith(tagPrefix)
      // release-please publishes a GitHub Release for EVERY version bump the
      // moment its PR merges, but the signed macOS assets are attached LATER by
      // the manual desktop-release workflow (and some versions are skipped and
      // never built at all). Such a release is "the newest" yet has no
      // downloadable asset — serving it makes the website's /download 404 and the
      // native updater 204 ("can't update"). Require a real platform asset so we
      // fall through to the newest release that was actually built.
      const hasPlatformAsset =
        Array.isArray(item.assets) &&
        item.assets.some(asset => checkPlatform(asset.name))
      return !item.draft && isPre && prefixOk && hasPlatformAsset
    })
    ...
    // Our release-please tags are prefixed (`desktop-vX.Y.Z`), which is NOT valid
    // semver — Hazel's update route does `semver.compare(latest.version, …)` and
    // THROWS on a bad version, hanging the request. Store the clean `X.Y.Z` (the
    // desktop app's `app.getVersion()` is unprefixed, so they compare correctly).
    const version = semver.valid(semver.coerce(tag_name)) || tag_name
    ...
    console.log(`Caching version ${version} (from tag ${tag_name})...`)
